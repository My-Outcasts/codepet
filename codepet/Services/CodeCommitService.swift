...
enum CodeCommitService {
    ...
    static func beginGit(projectPath: String, taskTitle: String) -> GitSession? {
        ...
        // A leftover branch from an earlier run of the SAME ask (repo left parked on
        // `codepet/<slug>` because that run wasn't finalized) must not fail this one.
        // Pick the first free name: `codepet/<slug>`, then `-2`, `-3`, … so re-running
        // the same ask always starts a clean branch instead of colliding.
        let base = "codepet/" + CommitSlug.make(from: taskTitle)
        var branch = base
        var n = 2
        // `rev-parse --verify --quiet <branch>` succeeds iff the branch already exists.
        while GitRunner.run(["rev-parse", "--verify", "--quiet", branch], in: projectPath).ok {
            branch = "\(base)-\(n)"
            n += 1
        }
