...
/// real `claude` subprocess. Production conformer is `CLIRunAdapter`.
...
/// Bridges `CLIRunner` (an ObservableObject that streams to `@Published`
...
@MainActor
final class CLIRunAdapter: CodeRunning {
        ...
        return await withCheckedContinuation { (cont: CheckedContinuation<CodeRunOutcome, Never>) in
            ...
            stateCancellable = runner.$state.sink { state in
                ...
                case .finished:
                    ...
                    // CLIRunner.computeDiffs), so this read is complete.
