...
final class CodeCommitServiceTests: XCTestCase {
    ...
    func test_beginGit_uniquifiesWhenBranchAlreadyExists() {
        // A leftover branch from an earlier run of the same ask must not fail the
        // next run — it gets the next free name (`-2`, `-3`, …) instead of colliding.
        let repo = tempGitRepo()
        let first = CodeCommitService.beginGit(projectPath: repo, taskTitle: "edit code")!
        XCTAssertEqual(first.branch, "codepet/edit-code")
        _ = GitRunner.run(["checkout", first.originalRef], in: repo)   // leave the branch behind

        let second = CodeCommitService.beginGit(projectPath: repo, taskTitle: "edit code")
        XCTAssertEqual(second?.branch, "codepet/edit-code-2")
        XCTAssertEqual(currentBranch(repo), "codepet/edit-code-2")

        _ = GitRunner.run(["checkout", second!.originalRef], in: repo)
        let third = CodeCommitService.beginGit(projectPath: repo, taskTitle: "edit code")
        XCTAssertEqual(third?.branch, "codepet/edit-code-3")
    }
