...
/// must stay with the host.
///
/// This is the direction that actually hurts. A miss is invisible — the host answers, which is
/// what happened before this feature existed. A false positive puts the wrong pet's name on an answer,
...
final class DepartmentLexiconTrapTests: XCTestCase {
    ...
    func testEverydaySenseOfASalesWordDoesNotRoute() {
        assertStaysWithTheHost("that was a good deal on the laptop", "\"deal\" in its everyday sense")
        assertStaysWithTheHost("let me demo it to you quickly", "\"demo\" can be showing anyone anything")
    }

    /// The pets have names, and one of them is ordinary engineering vocabulary.
    func testAPetsOwnNameDoesNotRouteToItsDepartment() {
        assertStaysWithTheHost("crash is my favourite pet", "\"crash\" is a pet's name")
    }

    /// Generic founder talk — the most common thing typed into this box.
    func testGenericFounderTalkStaysWithTheHost() {
        assertStaysWithTheHost("what should I focus on this week", "no department named or implied")
        assertStaysWithTheHost("can you summarize where we are", "a request to the host")
        assertStaysWithTheHost("give me a plan for the next two weeks", "planning is the host's job")
        assertStaysWithTheHost("this is taking longer than I thought", "a feeling, not a domain")
        assertStaysWithTheHost("what did I decide about this last month", "a memory lookup")
    }
    ...
    func testTheAugustRegressionShapesStayDead() {
        assertStaysWithTheHost("the app is well designed", "\"designed\" must not match \"design\"")
        assertStaysWithTheHost("I am happy with design so far", "a department NAME merely mentioned")
    }
    ...
    func testAWordEndingInSIsNotTreatedAsAPlural() {
        assertStaysWithTheHost("our focus is unclear", "\"focus\" is not the plural of anything")
    }

    /// Genuinely two-department sentences fail the margin and stay with the host, rather than the
    ...
    func testAGenuinelyAmbiguousSentenceStaysWithTheHost() {
        assertStaysWithTheHost("can I use this font commercially", "legal and design tie on the margin")
    }
}
