...
- **Every `xcodebuild` invocation must also set `dangerouslyDisableSandbox: true` on the Bash tool call.** Sandboxed, `codesign` cannot reach the login keychain, and the build dies with `Command CodeSign failed with a nonzero exit code` on the embedded framework dylibs (`libXCTestSwiftSupport.dylib`, `openssl_grpc.framework`, …) — no `Failing tests:` line, no `Executed N tests` line, so it looks like the Firestore-lock symptom but is not. Verified on 2026-08-03: identical command fails sandboxed, succeeds unsandboxed. Do not "fix" this by adding an `xcodebuild` allow rule — building runs arbitrary project scripts, so a standing pre-approval is exactly what the permission posture is meant to avoid.
