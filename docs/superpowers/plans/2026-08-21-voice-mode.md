...
- ~~**Enabling voice processing changes the input format to 7ch / 48kHz / deinterleaved.** Never tap `inputNode` directly for recognition — route through an `AVAudioMixerNode` with an explicit format.~~ **WRONG, and it was the bug — corrected 22 Aug.** Voice processing does change the input format to multi-channel (3ch/24000, 5ch/48000 and 7ch/48000 all measured on one Mac; it is renegotiated per session, not a constant), and `AVAudioConverter(7ch→1ch)` does report `channelMap == [-1]` and does output all zeros. But the mixer this justified is never rendered when nothing is connected downstream, so the tap on it delivered zero-filled buffers at the right rate for days with no error. **Tap `inputNode` directly with an explicit 1ch format at the input bus's own sample rate** — `installTap` does the downmix itself, `AVAudioConverter` was never needed, and any other sample rate makes `engine.start()` throw `-10875`. See the superseded block in the design spec §"And the spike found a landmine" and `.superpowers/sdd/tap-fix-report.md`.
