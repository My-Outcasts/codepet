...
This plan is committed; the SDD ledger it was executed against (`.superpowers/sdd/2026-08-22-two-tier-memory/progress.md`) is not. Anything the controller ruled differently from what is written above lives here so it survives the ledger's deletion.

- **Task 3's map became account-keyed, not the flat `[path: id]` this plan specifies.** A project id only means something inside one founder's `companies/{uid}`, so a single flat map has no correct behaviour on sign-out: clearing it on sign-out orphans the same founder's bindings on sign-in, and keeping it leaks one founder's bindings to the next. `ProjectIdentityMap` ships as `[companyId: [path: id]]` with a mutable `account: String?` that `CompanyStore` sets on `hydrate` and clears on `reset()` — every lookup and bind is a no-op while `account` is nil. `id(forPath:)`/`bind(path:to:)` keep their names but are account-scoped underneath.

- **Task 5's helper signature was two incompatible calls in this plan (`makeStore(cloudProjects:)` and `makeStore(cloudProjects:forcedRemote:cloud:)`); it shipped as one: `makeStore(cloud: [CloudProject] = [], forcedRemote: String? = nil, forcedRoot: String? = nil)`.** `forcedRoot` was added during the fix wave, past what this plan anticipated at all — see the `isGitRepo` item below.

- **The injected cloud-project reader is synchronous, not the `async` closure this plan's interfaces note suggested.** `knownCloudProjects` ships as a plain stored `[CloudProject]` (a `let` as of the fix wave — nothing mutates it in PR 1), empty until PR 2's cloud sync fills it. Making `linkProject` async to support an async reader would have touched its four view call sites (`ChatComposer`, `EnvironmentView`, `TasksView`, `RoadmapView`) to buy nothing in PR 1, where the list is empty by construction and only `.mint` can run.

- **`reload()` does not exist.** This plan's Task 3 lists a `func reload()` and Task 5 instructs calling `identityMap.reload()` wherever `PetMemoryStore.shared.reload()` runs on account switch (`:2324`) — but `PetMemoryStore.shared.reload()` is not called from `CompanyStore` at all; that instruction pointed at the wrong file. What ships instead is the `account` pointer: `CompanyStore.hydrate` sets `identityMap.account = companyId`, and `reset()` sets it to `nil`. A `reload()` method was written during implementation (re-read the map from disk, discarding memory) but had no caller and no test, so the PR 1 fix wave deleted it. `resetAll()` (wipe every account's bindings, unrelated to sign-out) stays — it has no caller either, but it is tested, and it is documented in-file as the deliberate-full-reset escape hatch a later PR is expected to wire up.

- **The `isGitRepo` gate on reading the remote was removed, then its replacement (`repoRoot`) was added in a fix round.** This plan's Task 5 (line ~828) reads the remote only when `link.isGitRepo`. The shipped code reads the remote unconditionally — `link.isGitRepo` only checks for a `.git` directory in the exact linked folder, so gating on it made `.propose` unreachable in Task 5's own tests, which inject a forced remote against a non-git temp folder. That was accepted as correct (the plan's gate was wrong, not the implementation) until a later review caught the real hole it left open: `git remote get-url origin` WALKS UP to find a repo, so a folder nested inside a tracked ancestor borrowed the ancestor's remote and could be proposed as the ancestor's project — a wrong question put in front of the founder with plausible-looking evidence attached. The fix, added in a fix round rather than planned here: `GitRunner.repoRoot(in:)` (`git rev-parse --show-toplevel`) and a new `isRoot` check — the remote is only read as a hint when the linked folder IS the repo root. `CompanyStore` gained a `repoRootReader` closure alongside `remoteURLReader`, following the same injectable-closure pattern.

- **Not written down here at the time, fixed in the PR 1 fix wave: the `isRoot` comparison itself needed canonicalising.** It compared the linked folder's raw path against `repoRoot`'s output, which `git rev-parse` canonicalises (resolves symlinks) — so a linked folder reached through a symlinked ancestor could genuinely BE the repo root and still fail the comparison, silently dropping a legitimate remote hint. `linkProject` now canonicalises the incoming path once, with `URL(fileURLWithPath:).resolvingSymlinksInPath().path`, and uses that canonical path for the `isRoot` comparison, the identity map key, and everything else downstream in the function.
