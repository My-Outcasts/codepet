...
final class DayOneBridgeTests: XCTestCase {
    ...
    /// Savers injected: a `CompanyStore` reaching Firestore with no `FirebaseApp` TRAPS.
    private func dayOneStore() -> CompanyStore {
        let store = CompanyStore(
            tasksSaver: { _, _ in true },
            librarySaver: { _, _ in true },
            decisionsSaver: { _, _ in true },
            firstApprovalSaver: { _, _ in true })
        store.company.tasks = DemoProject.murrorDayOne.tasks
        store.company.library = DemoProject.murrorDayOne.library()
        return store
    }

    /// Walk the chain the way the script does: record link 1, then file each next link's
    /// authored artifact in beacon order.
    private func runTheNineQuestions(_ store: CompanyStore) async {
        ...
        for id in DemoProject.dayOneChain {
            ...
            if task.who == .you {
                await store.recordFounderOutcome(taskId: id, body: body, kind: .doc)
            } else {
                store.company.tasks[store.company.tasks.firstIndex { $0.id == id }!].draft =
                    Deliverable(kind: DeliverableKind(raw: entry.kind), title: task.title,
                                body: body, sourceTaskId: id)
    ...
    /// **The hand-off, at the mechanism rather than the caption.** Each link after the first
    /// must be able to read its predecessor's filed work, or the credit line renders empty.
    func testEachLinkCanReadItsPredecessorsWork() async {
        let store = dayOneStore()
        await runTheNineQuestions(store)
        for (i, id) in DemoProject.dayOneChain.enumerated() where i > 0 {
            guard let task = store.company.tasks.first(where: { $0.id == id }) else { continue }
            let upstream = UpstreamWork.assemble(for: task, in: store.company.tasks,
                                                 library: store.company.library)
            XCTAssertFalse(upstream.isEmpty,
                           "\(id) credits nothing — the hand-off is invisible at this link")
        }
    }

    /// Link 1 included: the interviews are `.you`, and the whole chain reads them.
    func testTheFoundersOwnWorkIsReadableByTheChain() async {
        let store = dayOneStore()
        await runTheNineQuestions(store)
        guard let landscape = store.company.tasks.first(where: { $0.id == "mur-landscape" })
        else { return XCTFail("mur-landscape missing") }
        let upstream = UpstreamWork.assemble(for: landscape, in: store.company.tasks,
                                             library: store.company.library)
        XCTAssertTrue(upstream.contains { $0.taskTitle.contains("12 people") },
