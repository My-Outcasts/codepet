...
- **`chief_of_staff` is not rendered as the founder's companion pet**, which the contract's `department_key` mapping table asks for. It takes neutral ink (`CodepetTheme.mutedText`) like any other non-department role, for the same reason the cards do not use `AgentsWorkingRow`: the room's rows are deliberately not companion-centric. Recorded as a known deviation, not a decision to keep — it needs the same design answer as the challenger's identity.
