...
describe("handleCompanyChat", () => {
  ...
  describe("JSON path (no Accept: text/event-stream)", () => {
    ...
    test("run_task_id is the matched task id when the model calls run_task with a valid task_id", async () => {
      ...
      // always-on add_task, draft_message, navigate and remember_fact. `add_task` joined the
      // always-on set on Aug 8 and `draft_message` on Aug 10: neither has a precondition, unlike
      // run_task (needs something runnable) and complete_task (needs an open founder-owned task).
      const call = mockMessagesCreate.mock.calls[0][0] as any;
    ...
    test("only the always-on add_task + draft_message + navigate + remember_fact tools are offered, and run_task_id is null, when runnable/env_setup/open_tasks are omitted (backward compat)", async () => {
      ...
      expect(body).toEqual({ reply: "Hello founder.", run_task_id: null });
      const call = mockMessagesCreate.mock.calls[0][0] as any;
      expect((call.tools as any[]).map((t) => t.name)).toEqual(["add_task", "draft_message", "navigate", "remember_fact"]);
    });
