/**
 * Everything the model calls are MADE of — prompts, tool schemas, prompt builders, model ids,
 * pricing — with nothing that talks to a model.
 *
 * Split from `anthropic.ts` so the local path can reach these. `local/oneShotSidecar` is
 * esbuild-bundled into the app and inlines its whole import graph, so a builder reachable only
 * through a module that imports the Anthropic SDK ships the SDK (measured elsewhere in this
 * work: 7.5 MB of app resource). The four functions that hold an `Anthropic` client stayed
 * behind in `anthropic.ts`, which re-exports this file so all 29 importers are unchanged.
 */
...
// ─── One assembly per phase, shared by every transport ───────────────────────
//
// These four were inlined in `callAnthropic` / `streamAnthropic` /
// `callAnthropicSession` / `streamAnthropicSession` — four copies of two assemblies, which
// was survivable while the API was the only caller. It stopped being survivable when the
// founder's own Claude Code became a second caller: a prompt edited for one and not the
// other is a narrative in the wrong language on one transport only.

/** The system + user text for one turn narrative. */
export function narrativeRequest(args: CallArgs): { system: string; user: string } {
  return {
...
/**
 * A narrative if every field the client renders is present, otherwise null.
 *
 * `detected_skills` is required here but NOT by `callAnthropic`'s own check — the streaming
 * path always required it and the non-streaming one never did. Requiring it everywhere is the
 * stricter of the two behaviours, and the client reads the field either way.
 */
export function coerceNarrative(input: unknown): NarrativeOutput | null {
  const n = input as NarrativeOutput | null;
  if (!n || typeof n !== "object") return null;
  const ok =
    typeof n.title === "string" &&
    typeof n.what_you_wanted === "string" &&
    typeof n.what_happened === "string" &&
    typeof n.lesson === "string" &&
    typeof n.next_steps === "string" &&
    typeof n.mood === "string";
  if (!ok) return null;
  return { ...n, detected_skills: Array.isArray(n.detected_skills) ? n.detected_skills : [] };
}

/** The system + user text for one session summary. */
export function sessionSummaryRequest(args: SessionCallArgs): { system: string; user: string } {
  return {
    system: SESSION_SYSTEM_PROMPT
      .replace("<language>", args.language === "vi" ? "Tiếng Việt" : "English")
      .replace("<persona_block>", renderPersonaBlock(args.petPersona)),
    user: buildSessionUserMessage(args.turns, args.userBrief, args.petMemory),
  };
}

/** A session summary if it carries the two fields the client cannot render without. */
export function coerceSessionSummary(input: unknown): SessionSummaryOutput | null {
  const s = input as SessionSummaryOutput | null;
  if (!s || typeof s !== "object") return null;
  if (typeof s.summary !== "string" || typeof s.lesson !== "string") return null;
  return {
    summary: s.summary,
    lesson: s.lesson,
    brief_update: typeof s.brief_update === "string" ? s.brief_update : undefined,
    project_overview: typeof s.project_overview === "string" ? s.project_overview : undefined,
    growth_signals: Array.isArray(s.growth_signals) ? s.growth_signals : undefined,
  };
}
