/**
 * Everything `generateDictionary` is MADE of — the payload contract, the companion's system
 * prompt, the forced tool, and the user message — with nothing that talks to a model and
 * nothing that touches the Firestore term cache.
 *
...
export const DICTIONARY_SYSTEM_PROMPT = `You are the user's coding companion — a pet character building the user a PERSONAL dictionary from the terms that show up in THEIR OWN code. For each term, you write a small card that makes it click for a beginner, grounded in their actual project.
...
export function buildDictionaryUserMessage(payload: GenerateDictionaryPayload): string {
  let projectBlock = "";
  if (payload.project) {
...
/** The system + user text for one batch of dictionary cards. */
export function dictionaryRequest(
  payload: GenerateDictionaryPayload,
  terms: TermRequestInput[] = payload.terms
  ...
  return {
    ...
    // Only the terms asked for: the cloud path passes its cache MISSES here, the local path
    // passes all of them, and neither should pay for a card it already has.
    user: buildDictionaryUserMessage({ ...payload, terms }),
  };
}

/** The entries a reply carries, or null when it carries no list at all. */
export function coerceDictionaryEntries(input: unknown): DictionaryEntryOutput[] | null {
  const entries = (input as { entries?: unknown } | null)?.entries;
  return Array.isArray(entries) ? (entries as DictionaryEntryOutput[]) : null;
}

/**
 * Map generated entries back onto the terms that were asked for.
 *
 * By term first, then by position — the model echoes the token but can normalise its case,
 * and the client maps cards to its own tokens exactly. The REQUESTED spelling wins, which is
 * the handler's rule kept in one place so the local path cannot map them differently.
 */
export function alignEntries(
  requested: TermRequestInput[],
  entries: DictionaryEntryOutput[]
): DictionaryEntryOutput[] {
  const byTerm = new Map<string, DictionaryEntryOutput>();
  for (const e of entries) {
    if (e && typeof e.term === "string") byTerm.set(e.term.toLowerCase(), e);
  }
  const out: DictionaryEntryOutput[] = [];
  requested.forEach((term, k) => {
    const entry = byTerm.get(term.term.toLowerCase()) ?? entries[k];
    if (!entry) return;
    out.push({ ...entry, term: term.term });
  });
  return out;
}
