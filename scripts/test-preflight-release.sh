...
#
# The two lookups it performs are indirected through $IDENTITY_CMD and $NOTARY_CMD so this
# can feed fixtures. Without that seam the only way to test the failure path would be to
# revoke a certificate — so the failure path would never be tested, and the failure path is
...
check() {
  local label="$1" expected="$2" ids="$3" notary="$4" out actual
  out="$(IDENTITY_CMD="cat $ids" NOTARY_CMD="cat $notary" "$SCRIPT" 2>&1)"
  actual=$?
...
says() {
  local label="$1" ids="$2" notary="$3" needle="$4" out
  out="$(IDENTITY_CMD="cat $ids" NOTARY_CMD="cat $notary" "$SCRIPT" 2>&1)"
